...
def get_argparser():
    ...
    parser.add_argument(
        "-s", "--server", action="append", help="Add a WAnD server by IP address"
    )
    parser.add_argument(
        "-poll",
        "--poll-time",
    ...
    influx.add_argument(
        "--user-db", help="InfluxDB username (default: '%(default)s')", default="admin"
    )
    influx.add_argument(
        "--password-db",
    ...
    parser.add_argument(
        "--timeout",
        help=(
            "timeout for RPC connection to servers, in seconds "
            + "(default: %(default)s s)"
        ),
        type=float,
        default=5.0,
        ...
        for server in args.server:
            try:
                ...
                for laser in lasers:
                    ...
                    logger.info(
                        "{}: freq {} THz, f_ref {} THz, detuning {} MHz".format(
